...
**Phase 3 opened at the browser layer, 2026-08-06 — for one of its three risks.**
Playwright landed (§4) and two specs came with it: `e2e/vote-link-oracle.spec.ts`
covers **#7**, and `e2e/units-import.spec.ts` covers the user-visible half of **#8**.
`e2e/seed.spec.ts` is the convention exemplar the other two are modelled on. **#5 and
#9 are untouched**, which is why the Status cell reads `in progress` and not
`complete` — the two-step confirm path, the thing §5's conditional e2e gate was
written for, still has no test at any layer.

This promotion needs justifying against §1's first principle, because it runs
against it: §2 assigns #7 to `integration`, and a cheaper layer could have reached
it. The honest account is that the browser test was written as a deliberate
exercise, and that it earns its keep only because of how it asserts — it compares a
resolving token against four shapes of miss on **status, the three observable
headers, the redirect chain, and the response bytes**, rather than on rendered
markup. §2 names "asserting only the body" as this risk's anti-pattern, and a DOM-
reading browser test would have walked into it. A future contributor deciding
whether to promote something else should read this as the bar, not as permission.

**What the #8 spec does not do: replace Phase 2.** It asserts what an administrator
...
| e2e                  | **Playwright** — `npm run test:e2e`                                             | @playwright/test 1.62.1, chromium only         | Installed 2026-08-06. Config is `playwright.config.ts`; specs live in `e2e/`, fixtures in `e2e/fixtures/db.ts`. Runs against the real local stack and the real workerd dev server — nothing is mocked, which is the point, since every risk it covers lives in a crossing. One session is signed in once by the `setup` project and shared through `storageState`; `playwright/.auth/` is gitignored. Adopted for **#7 and the visible half of #8**, not as a default layer: §1's cost×signal rule still governs, and §3 records why this promotion was allowed; checked: 2026-08-06                                                                                                                                                             |
...
| e2e on the confirm flow                   | CI on PR                                                     | still conditional — see the note                                  | the two-step confirm crossing, if integration cannot reach it. **Read this row carefully after 2026-08-06:** e2e now exists in the project, so "only if §3 Phase 3 promotes" no longer reads as a blocker — but the promotion that happened was for #7. The confirm flow (#5) has no test at any layer, and this gate is still waiting on one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| e2e suite                                 | local (`npm run test:e2e`) + CI (`ci.yml`, `e2e` job)        | **required — wired 2026-08-06**                                   | a voting link answering differently for a real token, an unknown one, a draft uchwała's one and a truncated one, on status, headers, redirect or response bytes (#7); a refused registry import leaving a budynek half-populated or permanently locked as the administrator sees it (#8, visible half). Every assertion in both specs was checked by breaking the code under it. Known limit: the suite needs Docker and a migrated local stack, exactly as `db-contract` does, and it asserts what the browser can observe — the transactional guarantee under #8 is still Phase 2's |
...
job of its own; three specs live in `e2e/`. Consequential edits: §3 (Phase 3 Status
`not started` → `in progress`, its Test types cell, and two new paragraphs — why this
promotion was allowed against §1's cost×signal rule, and why the #8 spec does not
discharge Phase 2); §4 (the `e2e` row, which said "Neither is installed and neither
is adopted by default" and had said so since 2026-08-04); §5 (a new required `e2e
suite` gate, and a warning added to the `e2e on the confirm flow` row, which stays
conditional).

Two things this entry deliberately does **not** claim. Phase 3 is not complete — #5
and #9 have no test, and #5 is the confirm path the conditional gate was written
for. Phase 2 is not touched — the browser can see that a refused import left no
registry, but not that the write was atomic, and those are different assertions
about different layers.

**§7 is unchanged and that is deliberate.** Its first exclusion covers "no snapshot
or pixel-diff tests on pages, Tailwind classes, or generated components", and none
of the three specs does any of that: they drive user paths and assert domain
outcomes and HTTP responses. The exclusion is about rendering, not about the
browser as an instrument. If a pixel-diff or a Tailwind assertion ever gets proposed
on the back of "we have Playwright now", that is the line it crosses.
