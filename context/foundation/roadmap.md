...
| ---- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------- | ---------------------------- | -------- |
| F-01 | `production-admin-access`   | (fundament) administrator ma na produkcji działające konto i logowanie, a brak sekretu przestaje być cichy               | —             | Access Control, Guardrails   | ready    |
...
- **Auth:** partial — klient Supabase SSR (`src/lib/supabase.ts`), middleware ustawia `context.locals.user`, bramka `PROTECTED_ROUTES = ["/dashboard"]` w `src/middleware.ts`, endpointy `signin`/`signup`/`signout`. Ograniczenia: sekrety są `optional: true`, więc brak konfiguracji daje `null` client i ciche no-opy; Site URL w Supabase nieustawiony, więc linki potwierdzające prowadzą pod zły origin; ścieżka rejestracja → potwierdzenie → logowanie nigdy nie została przeprowadzona na produkcji.
- **Deploy / infra:** present — Cloudflare Workers, aplikacja żyje pod `estate-manager.estate-manager.workers.dev`; `.github/workflows/ci.yml` (lint + build) oraz `deploy.yml` (auto-deploy przy push do `main`, opublikował dwa razy na zielono).
...
- **Outcome:** (fundament) konto administratora da się założyć w trybie wdrożeniowym i zalogować się nim na produkcji, a brak wymaganej konfiguracji Supabase przestaje przechodzić jako zielony deploy.
- **Change ID:** `production-admin-access`
- **PRD refs:** `## Access Control` (administrator jest jedyną rolą, która się loguje; konto zakładane przy wdrożeniu, bez samodzielnej rejestracji), `## Success Criteria` → Guardrails (nikt spoza rejestru nie oddaje głosu)
...
- **Blockers:** —
- **Unknowns:**
  - Czy konto administratora zakładamy przez potwierdzenie e-mailem (wymaga ustawienia Site URL), czy przez ręczne utworzenie użytkownika w panelu Supabase z pominięciem potwierdzenia? — Owner: użytkownik. Block: no.
- **Risk:** Sekrety zadeklarowane jako `optional` były świadomym wyborem startera i są niebezpieczne od momentu, w którym aplikacja przestaje być szkieletem: rotacja klucza albo deploy z powłoki bez bindingów daje Worker zwracający 200 i puste strony. Sekwencjonowany jako pierwszy, bo cel „sygnał od użytkowników" zakłada obserwowanie produkcji, a nie środowiska lokalnego. Zakres celowo minimalny — to nie jest przebudowa uwierzytelniania, tylko doprowadzenie istniejącej ścieżki do stanu sprawdzalnego i głośno zawodzącego.
- **Status:** ready
...
- **Samodzielna rejestracja administratora** — Why parked: PRD §Non-Goals; konto zakładane przy wdrożeniu. Konsekwencja dla `F-01`.
