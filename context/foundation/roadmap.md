...
- **Do przemyślenia poza tym kawałkiem:** `/vote/<token>` nie ustawia nagłówków `Cache-Control` ani `X-Robots-Tag` (świadomie pominięte; kto to zmieni, musi ustawić je **przed** rozgałęzieniem na trafiony/nietrafiony token, inaczej nagłówki same staną się wyrocznią odróżniającą tokeny). `opened_at` zapisuje zegar Workera, nie bazy — `S-05` nie może odejmować go od znacznika generowanego przez bazę bez wcześniejszego rozstrzygnięcia. Workers Logs zapisuje adres każdego żądania, więc tokeny są utrwalane przez 7 dni po stronie Cloudflare i żadna zmiana w tym repozytorium tego nie zmienia.
...
- **Blockers:** —
- **Unknowns:**
  - PREREKWIZYT Z PRZEGLĄDU `S-02` 2026-08-02: zamrożenie uchwały pilnuje dziś wyłącznie `update ... resolutions`. **Nic nie odmawia** `update voting_links set owner_id = …` — przepisania żywego tokenu na inną osobę, co złożony klucz obcy dopuszcza w obrębie budynku — ani `delete from resolutions` na otwartej uchwale, co kaskadowo usuwa jej linki. Dopóki nie ma głosów, nie ma czego sfałszować; z chwilą, gdy `S-03` zaczyna zapisywać głosy, obie luki stają się drogą do podmiany głosującego. Do planu `S-03` wchodzą więc: wyzwalacz `before update` na `voting_links` odrzucający zmianę `token` / `owner_id` / `resolution_id` oraz odmowa `delete on resolutions` przy `status <> 'draft'`. — Owner: zespół. Block: **yes** — dla `S-03`.
  ...
  - Skąd `S-03` weźmie token do sprawdzenia ścieżki, skoro `S-02` nie dostarcza linku nikomu? Dziś: odczyt wprost z bazy rolą `postgres`. Sprawdzenie z prawdziwym właścicielem czeka na `S-04`. — Owner: zespół. Block: no.
...
| S-03       | `share-weighted-vote`          | Oddanie ważonego udziałem głosu z indywidualnego linku                     | yes                   | `S-02` zamknięte 2026-08-02, więc odblokowane. Gwiazda przewodnia. Prerekwizyty z przeglądu `S-02`: patrz **Unknowns** przy `S-03`                                                                |
