...
- [x] 2.1 `npx astro sync && npm run lint && npm run build` clean — 58156b7
- [x] 2.2 `grep -c '"/api/vote"' src/middleware.ts` → `0` — 58156b7
- [x] 2.3 No `console.` and no token interpolation in the new files — 58156b7

#### Manual

- [x] 2.4 Full local path: buttons → confirm → receipt with the right choice, date and weight — 58156b7
- [x] 2.5 Receipt persists across a reload and a fresh browser profile — 58156b7
- [x] 2.6 Second owner votes the other way; both weights match the registry — 58156b7
- [x] 2.7 Every neutral case renders identically with `200`, for every `?wybor` value — 58156b7
- [x] 2.8 All three headers present and identical between resolving and non-resolving tokens — 58156b7
- [x] 2.9 `curl` without `Origin` → `403`; with `Origin` and an unknown token → neutral page, no error parameter — 58156b7
- [x] 2.10 Forged `choice` refused without revealing whether the token exists — 58156b7
- [x] 2.11 The whole path works with JavaScript disabled — 58156b7

### Phase 3: The administrator's vote count

#### Automated

- [x] 3.1 `npx astro sync && npm run lint && npm run build` clean — b348359
- [x] 3.2 The `voting_links` projection still names columns and still excludes `token` — b348359

#### Manual

- [x] 3.3 With two of three linked owners voted, the page reads `Zagłosowało 2 z 3 właścicieli` — b348359
- [x] 3.4 The draft branch is unchanged and no token appears anywhere — b348359
- [x] 3.5 The count does not appear before the vote is opened — b348359
