...
- **Two slices of the PRD are built.** `S-01` (`building-create`) shipped 2026-08-02: an administrator creates a building at `/buildings/new` (name, city, street-with-number — PRD `FR-011`) and sees it listed at `/buildings`. `S-01b` (`building-units-import`) shipped the same day: from `/buildings/<id>` an administrator imports a CSV of lokale into a building whose registry is still empty — upload → preview with computed udziały → confirm — and lands on a registry totalling exactly 100,00%. Uchwały and głosowanie are still unbuilt (`S-02` onward). Records: `context/changes/building-create/`, `context/changes/building-units-import/`.
