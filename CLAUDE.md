...
- **Three test harnesses, and few tests — say which you ran, and never let "tests passed" stand in for "the domain is verified."** `npm test` runs Vitest over `src/**/*.test.ts`; `npm run test:db` runs pgTAP over `supabase/tests/database/*.test.sql`; `npm run test:e2e` runs Playwright over `e2e/*.spec.ts`. The last two need Docker plus the local stack up. All three are green, and between them they pin the udział allocation, the registry parse, the audit trail's arithmetic, `EM015`, voting-link hit/miss indistinguishability and what a refused import leaves behind **as the administrator sees it** — but **the database layer is still entirely unpinned**: nothing asserts the threshold, RLS, the electorate guards, or import atomicity as a transactional guarantee. A green `npm test` says nothing about any of them. Full verification is `npm run lint && npm test && npm run build`, plus `npm run test:db` for anything touching `supabase/` and `npm run test:e2e` for anything touching a route a browser reaches. Which risks are still open, and how to write the tests: `context/foundation/test-plan.md` §2, §6.1, §6.2.
...
- **E2E runs against a live stack and the `supabase/seed.sql` administrator — it cannot run on a database that was never seeded**, and it cleans up through `e2e/fixtures/db.ts` rather than the UI, because the product has no delete path at all. `purgeBuilding` steps outside the product as superuser with triggers off; it belongs in `afterEach` and nowhere else.
- **The `service_role` client in `e2e/fixtures/db.ts` is for setup and teardown only.** A test that reaches the behaviour under test through it asserts the database rather than the application — and every risk these tests exist for lives above the database.
